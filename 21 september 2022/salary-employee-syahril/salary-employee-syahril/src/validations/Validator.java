...
public class Validator {
  ...
  // Check limit Number min and max
  ...
  // Check employe Id available or not
  ...
  public static String makeEmployeeID() {
    ...
    do {
      for (Person data : data.allData) {
        if (data instanceof Employee) {
          String newdata = ((Employee) data).getEmployeeID();
          newdata = newdata.replaceAll("[^\\d.]", "");
          if (employeeid == Integer.parseInt(newdata)) {
            isRepeat = false;
          } else {
            isRepeat = true;
          }
