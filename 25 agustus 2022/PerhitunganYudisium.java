...
public class PerhitunganYudisium{
	...
	public static void main( String [] args){
		String 
		nama = "" ,predikat = "" , tempString;
		float 
		ipk = 0, jumlahBobot = 0, hasilIpk = 0;
		int 
		jumlahMataKuliah, jumlahSks = 0, minimalSks = 2, maximalSks = 6, minimalJumlahMatakuliah = 2, maximalJumlahMatakuliah = 12;
		boolean 
		isFindMataKuliah = true, isCheckInputMatakuliah = true, isCheckInputSks = true ,isCheckInputnilaiMatakuliah = true, repeat = true;

		DecimalFormat formatIpk = new DecimalFormat("#.##");
		...
		tempString = input.next();
		jumlahMataKuliah = validateInteger(tempString, isCheckInputMatakuliah, minimalJumlahMatakuliah, maximalJumlahMatakuliah);
		...
		do{
			for( int i = 0; i < jumlahMataKuliah; i++ ){
				...
				tempString = input.next();
				sksMatakuliah[i] = validateInteger(tempString, isCheckInputSks, minimalSks, maximalSks);
				
				//mencari bobot nilai dan validasi
				System.out.println(templateInputNilaiMataKuliah + mataKuliah[i]+ templateABC);
				tempString = input.next();
				nilaiMatakuliah[i] = validateStringNilai(tempString,isCheckInputnilaiMatakuliah);
		...
		System.out.println(templateIpk + " " + formatIpk.format(hasilIpk));
	}

	public static int validateInteger(String inputString, boolean repeat, int minimal, int maximal){
	...
	public static String validateStringNilai(String inputString, boolean repeat){
		while(repeat){
	...
	public static String findPredikat(String nilaiMatakuliah, float hasilIpk, String predikat ){
		float cumLaude = 3.5f, sangatMemuaskan = 2.75f, memuaskan = 2f;
