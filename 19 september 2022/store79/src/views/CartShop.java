...
public class CartShop {
  ...
  public CartShop(Product data, Integer quantity) {
  }
  ...
  public static CartItem addToCart(String idProduct) {
    ...
    do {
      ...
      Integer cartTotalProduct = getTotalCartShopByID(idProduct);
      Integer quantity;
      for (Product data : product) {
        ...
        if (quantity > 0) {
          ...
          if (!isLooping) {
            Integer indexProductCart = getIndexCartShopById(idProduct);
            if (cartTotalProduct != 0) {
              int totalBefore = getDataCartShop().get(indexProductCart).getQuantity();
              Double priceProduct = getDataCartShop().get(indexProductCart).getProduct().getPrice();
              getDataCartShop().get(indexProductCart).setQuantity(totalBefore + quantity);
              getDataCartShop().get(indexProductCart).setTotalPrice(priceProduct * (totalBefore + quantity));
            } else {
              addToCartShop(item);
            }
          }
  ...
  public static void updateDataProduct(List<CartItem> cartShop) {
    Integer jenisBarang = CART_SHOP.size();

    for (int i = 0; i < jenisBarang; i++) {
      for (Product product : repositories.Data.allData) {
        String idProduct = CART_SHOP.get(i).getProduct().getProductID();
        if (product.getProductID().equalsIgnoreCase(idProduct)) {
          if (product instanceof Items) {
            Integer stockTerbaru = ((Items) product).getUnitOfStock() - CART_SHOP.get(i).getQuantity();
            ((Items) product).setUnitOfStock(stockTerbaru);
            product.calculatedPrice();
          }
          if (product instanceof Services) {
            Integer userUsage = ((Services) product).getUserUsage() + CART_SHOP.get(i).getQuantity();
